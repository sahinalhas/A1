...
import { ApiError, ApiErrorCode, ApiErrorResponse, ValidationError, isApiErrorResponse } from '../../types/api-types';
...
export function createSafeApiCall<T, TArgs extends unknown[]>(
  ...
  return async (...args: TArgs): Promise<T | null> => {
    try {
      ...
      logger.error('API call failed', 'SafeAPICall', apiError);
      return null;
    }
  };
}
